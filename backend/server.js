...
io.use((socket, next) => {
  ...
  if (sessionID) {
    const session = userList.find(
      (user) => user.sessionID === sessionID && user.username == socket.username
    );
    if (session) {
      userList = userList.map((user) => {
        if (user.sessionID === sessionID && user.username == socket.username) {
          return {
            ...user,
            userID: socket.id,
          };
    ...
    connection.createChannel((err1, channel) => {
      ...
      channel.consume(
        ...
        (message) => {
          ...
          userList
            .filter((user) => user.sessionID !== data.user.sessionID)
            .forEach((user) => {
